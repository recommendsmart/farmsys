...
class LikeDislikeVoteBuilder implements LikeDislikeVoteBuilderInterface, TrustedCallbackInterface {
  ...
  public function build($entity_type_id, $entity_id) {
    ...
    $check_vote_init = $this->configFactory->get('like_and_dislike.settings')->get('check_vote_init');
    ...
    if (!$hide_vote_widget || $like_access) {
      ...
      if ($check_vote_init && (bool) $this->voteStorage->getUserVotes($this->currentUser->id(), 'like', $entity_type_id, $entity_id)) {
    ...
    if (!$hide_vote_widget || $dislike_access) {
      ...
      if ($check_vote_init && (bool) $this->voteStorage->getUserVotes($this->currentUser->id(), 'dislike', $entity_type_id, $entity_id)) {
